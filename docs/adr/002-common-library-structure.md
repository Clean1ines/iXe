...
## Контекст
В проекте наблюдается дублирование кода, моделей данных (например, `Problem`, `DBProblem`) и утилит (например, `subject_mapping`, `task_number_inferer`) между различными частями системы: скрапером (`scraper/`), веб-API (`api/`), сервисами (`services/`, `utils/`), процессорами (`processors/`) и скриптами (`scripts/`). Это приводит к:
...
- Изменения в `common` потенциально могут повлиять на несколько сервисов сразу, требуя более тщательного тестирования.
