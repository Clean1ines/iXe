...
class HTMLRenderer:
    ...
    def render_block_from_problem(self, problem: Problem, block_index: int) -> str:
        ...
        Prioritizes problem.offline_html for offline-ready rendering, falls back to problem.text.
        Args:
        ...
        try:
            ...
            # NEW: Use problem.offline_html if available, otherwise use problem.text
            if problem.offline_html and problem.offline_html.strip():
                logger.debug(f"Using problem.offline_html for problem_id {problem.problem_id}")
                processed_block_html_content = problem.offline_html
            else:
                logger.debug(f"Using problem.text for problem_id {problem.problem_id} (offline_html is empty or None)")
